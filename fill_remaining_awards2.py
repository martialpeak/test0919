#!/usr/bin/env python3
"""Fill remaining award notes using KNOWN_AWARDS mapped by persian names."""
import json

DB = '/root/moviebrowser-alpha/data/app_database.json'

# Known award data mapped by persian name
KNOWN_AWARDS = {
    'آن هاتاوی': [
...
for key, rec in a.items():
    ...
    # Get persian name (which is stored in 'name' field now)
    persian_name = rec.get('name', '')
    
    # Check if this actor has known awards data
